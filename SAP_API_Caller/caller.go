...
func (c *SAPAPICaller) Header(salesOrder string) {
	...
	itemData, err := c.callToItem(headerData[0].ToHeaderPartner)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callSalesOrderSrvAPIRequirementHeader(api, salesOrder string) ([]sap_api_output_formatter.Header, error) {
	...
	param := c.getQueryWithHeader(map[string]string{}, salesOrder)
...
func (c *SAPAPICaller) callToHeaderPartner(url string) ([]sap_api_output_formatter.HeaderPartner, error) {
...
func (c *SAPAPICaller) callToItem(url string) ([]sap_api_output_formatter.Item, error) {
	resp, err := c.requestClient.Request("GET", url, map[string]string{}, "")
	if err != nil {
		return nil, fmt.Errorf("API request error: %w", err)
	}
...
func (c *SAPAPICaller) Item(salesOrder, salesOrderItem string) {
	...
	itemPricingElementData, err := c.callToItemPricingElement(itemData[0].ItemPricingElement)
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	itemScheduleLineData, err := c.callToItemScheduleLine(itemData[0].ItemScheduleLine)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callSalesOrderSrvAPIRequirementItem(api, salesOrder, salesOrderItem string) ([]sap_api_output_formatter.Item, error) {
	...
	param := c.getQueryWithItem(map[string]string{}, salesOrder, salesOrderItem)
...
func (c *SAPAPICaller) callToItemPricingElement(url string) ([]sap_api_output_formatter.ItemPricingElement, error) {
	resp, err := c.requestClient.Request("GET", url, map[string]string{}, "")
	if err != nil {
		return nil, fmt.Errorf("API request error: %w", err)
	}
...
func (c *SAPAPICaller) callToItemScheduleLine(url string) ([]sap_api_output_formatter.ItemScheduleLine, error) {
	resp, err := c.requestClient.Request("GET", url, map[string]string{}, "")
	if err != nil {
		return nil, fmt.Errorf("API request error: %w", err)
	}
...
func (c *SAPAPICaller) getQueryWithHeader(params map[string]string, salesOrder string) map[string]string {
	if len(params) == 0 {
		params = make(map[string]string, 1)
	}
	params["$filter"] = fmt.Sprintf("SalesOrder eq '%s'", salesOrder)
	return params
}

func (c *SAPAPICaller) getQueryWithItem(params map[string]string, salesOrder, salesOrderItem string) map[string]string {
	if len(params) == 0 {
		params = make(map[string]string, 1)
	}
	params["$filter"] = fmt.Sprintf("SalesOrder eq '%s' and SalesOrderItem eq '%s'", salesOrder, salesOrderItem)
	return params
}
